use std::{collections::HashMap, fs, println, vec};

use crate::application::Ingestion::{embedder::SparseVecModel, pipeline::InjestRequest};
use qdrant_client::{
    Qdrant, qdrant::{
        CreateCollectionBuilder, Distance, PointStruct, SparseVector, SparseVectorBuilder, SparseVectorParamsBuilder, SparseVectorsConfigBuilder, UpsertPointsBuilder, Vector, VectorParamsBuilder, create_collection_builder, sparse_vector_builder, sparse_vectors_config, vector
    },
};
use serde::Serialize;
use serde_json::json;
...
pub async fn save_to_qdrant(
    ...
    for chunk in chunks {
        ...
        let qdrant_item: PointStruct = PointStruct {
            id: Some(chunk_ids[chunk_index].to_string().into()),
    ...
    if (chunker_type == "centroid".to_string()) {
        ...
        let insert_resp = qdrant_client
            .upsert_points(
                UpsertPointsBuilder::new("test_collection_centroid", qdrant_insert).wait(true),
            )
            .await;
...
pub async fn save_to_qdrant_sparse(
    injest_request: &InjestRequest,
    chunks: &Vec<String>,
    vectors: Vec<SparseVecModel>,
    parent_chunk_ids: Vec<Uuid>,
) -> Result<(), String> {
    let mut qdrant_insert: Vec<PointStruct> = Vec::new();
    let mut chunk_index = 0;
    for chunk in chunks {
        let payload_json: HashMap<String, serde_json::Value> = HashMap::from([
            ...
            (
                ...
                json!(parent_chunk_ids[chunk_index]),
            ),
            (
                "doc_name".to_string(),
                json!(injest_request.doc_name.clone()),
        ...
        let vector_sparse = &vectors[chunk_index]; 

        let mut sparse_vector = HashMap::new();
        

        //here we need to define a paired vector struct that the pointstruct expects for sparse vectors 


        sparse_vector.insert(
            "sparse".to_string(),
            Vector::new_sparse(vectors[chunk_index].indices.clone(), vectors[chunk_index].values.clone()),
        );
        ...
        let qdrant_item: PointStruct = PointStruct {
            ...
            payload: qdrant_payload.into(),
            vectors: Some(sparse_vector.into()),
        };
    ...
    sparse_config.add_named_vector_params("sparse", SparseVectorParamsBuilder::default()); //unlike for dense sparse vectors arent named by default

    let create_resp = qdrant_client
        .create_collection(
            CreateCollectionBuilder::new("test_collection_sparse")
    ...
    println!("sparse insert  = {:?}",qdrant_insert);

    let insert_resp = qdrant_client
        .upsert_points(UpsertPointsBuilder::new("test_collection_sparse", qdrant_insert).wait(true))
        .await;
