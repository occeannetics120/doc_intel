...
pub fn convert_to_string ( raw_file : Vec<u8>)  { 
        ...
        match raw_file_type {
              ft if ft.file_format().name.contains("Portable Document Format") => pdf_extractor(&raw_file), //passing an immutable borrow of raw_file so that we can reuse it or pass it down the pipeline or for logs 
